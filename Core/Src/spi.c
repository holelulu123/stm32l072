...
void SPI_Write(__uint8_t *message, __uint8_t rw_flag){
    /*
    ...
    */
    ...
    while (*message){
        ...
        SPI1->DR |= *message; // Write char (8 bit) to the TX Buffer (8b-dataframe).
        message++; // Itirate the pointer to the next address for the next word.
    }
    if (rw_flag){
